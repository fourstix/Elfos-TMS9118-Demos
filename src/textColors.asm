...
main:       call SET_GROUP     ; set group for video card
            call INIT_VREG     ; start with white text on blue background
...
COLOR_LOOP: call SET_TEXTCOLOR 
            call RESET_GROUP   ; set group back to default for Serial card
            mov  rf, 0h
            call f_read        ; f_read puts byte in D
            plo  rf            ; so store in rf.0
            smi  078h
            lbz  QUIT          ; exit if 'x' typed
            call SET_GROUP     ; set group back to video card
            glo  rf
            lbz  NO_NUM
